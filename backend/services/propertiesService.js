/**
 * @file services/propertiesService.js
 * @description
 */

const { PROPERTY_MAX_IMAGES } = require('../config/upload');
...
function getNormalizedUniqueImageUrls(urls) {
	if (!Array.isArray(urls)) {
		return [];
	}

	return Array.from(
		new Set(urls.map(normalizeImageUrl).filter((url) => url !== '')),
	);
}

function countTotalPropertyImages(cover, pictures) {
	const normalizedCover = normalizeImageUrl(cover);
	const normalizedPictures = getNormalizedUniqueImageUrls(pictures);
...
function validatePropertyImagesCount(cover, pictures) {
	const totalImages = countTotalPropertyImages(cover, pictures);

	if (totalImages > PROPERTY_MAX_IMAGES) {
		const error = new Error(
			`A property can contain at most ${PROPERTY_MAX_IMAGES} image(s), including cover`,
		);
		error.status = 400;
		throw error;
	}
...
async function ensureUniqueSlug(db, base, excludeId = null) {
	...
	while (true) {
		const row = excludeId
			...
			: await db.getAsync('SELECT id FROM properties WHERE slug = ?', [
					slug,
				]);
		if (!row) return slug;
		slug = `${base}-${n++}`;
...
async function getPropertyDetails(db, id) {
  ...
  `,
	...
	const pictures = await db.allAsync(
		'SELECT url FROM property_pictures WHERE property_id = ?',
		[id],
	);
	const equipments = await db.allAsync(
		'SELECT name FROM property_equipments WHERE property_id = ?',
		[id],
	);
	const tags = await db.allAsync(
		'SELECT name FROM property_tags WHERE property_id = ?',
		[id],
	);
	return {
		...base,
...
async function createProperty(db, payload) {
	...
	validatePropertyImagesCount(cover, pictures);
	...
	if (Array.isArray(normalizedPictures)) {
		for (const url of normalizedPictures) {
			if (url) {
				await db.runAsync(
